...
def lenslistinit():
    global l_HFID,l_fov,l_dmap,l_kmap,l_deltat,l_mu,l_srctheta,l_srcbeta,l_tancritcurves,l_caustic,l_einsteinradius
    l_HFID=[]; l_fov=[]; l_dmap=[]; l_kmap=[]; l_deltat=[]; l_mu=[]; l_srctheta=[]; l_srcbeta=[]; l_tancritcurves=[]
...
def lensing_signal():
    ...
    zl = redshift
    #TODO: remove hard-coding of zs=source redshift
    zs = 0.409
    ...
    for ff in range(len(dmfile)):
        ...
        for ll in range(len(dmf['HFID'])):
            ...
            #Rein = lt.einstein_radii_proper(kappa, cosmo)
    ...
    if args["lenses"] == True:
        ...
        tree['zl'] = redshift
        tree['zs'] = zs
        tree['FOV'] = l_fov  #[arcsec]
        tree['DMAP'] = l_dmap  #[arcsec]
        tree['KMAP'] = l_kmap  #[arcsec]
