...
def Einstein_ring(veldisp, c, z, Dls, Ds):
    """
    ...
    #    R_E = A_E.to_value('rad')*cosmo.angular_diameter_distance(z).to_value('Mpc')
    #    return R_E
    #if dim == 'rad':
    #else:
    #    print('Unsupported physical units')
...
def Einstein_Volume(fov, l_dist, z_dist):
    ...
    for j in range(len(z_dist)):
        ...
        V[j] = rfov[j]**2*z_dist[j]  # for Subfind
    return V, rfov
...
#@nb.njit  #(fastmath=True, parallel=False)
def SNIa_position(i, indx, SNIa_num, dist_sr, u_lenspos, l_lenspos, fov_Mpc,
                  S_ID, S_possky):
    ...
    for y in indx[0]:
        ...
        for x in range(int(SNIa_num[y])):
            ...
            sposx = np.sqrt(radial_rnd[0]**2 + radial_rnd[1]**2 + radial_rnd[2]**2)#[Mpc]
            zs = z_at_value(cosmo.comoving_distance, sposx*u.Mpc, zmax=2)
            # max. distance equa to re
            charge = 1 if rnd.random() < 0.5 else -1
            sposy = charge*rnd.random()*fov_Mpc[y]*0.5  #[Mpc]
            charge = 1 if rnd.random() < 0.5 else -1
            sposz = charge*rnd.random()*fov_Mpc[y]*0.5  #[Mpc]
            # Write glafic file
            sid[indxtot] = i
            sred[indxtot] = zs
            sx[indxtot] = sposx
